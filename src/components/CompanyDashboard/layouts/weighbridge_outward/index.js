...
function WeighBridge() {
  ...
  const toggleImage = (id, hideOverlay = false) => {
    ...
    setOpenModal(!hideOverlay); // Toggle modal state
  ...
  const renderImages = (detail) => {
    if (detail.images && showImages[detail.id]) {
      return (
        <div className={`image-popup ${openModal ? 'modal-open' : ''}`}>
...
.image-popup {
  position: absolute;
  top:-72% !important;/* Adjust as needed */
  left: 50%;
...
@media (max-width: 600px) {
  ...
  .image-slider {
    ...
    padding-bottom: 60px; /* Increased padding for visual separation */
